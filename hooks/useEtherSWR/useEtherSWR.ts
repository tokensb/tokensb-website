...
import EthSWRConfigContext from './ethSWRConfig';
import { etherJsFetcher } from './etherJsFetcher';
...
// @ts-ignore
function useEtherSWR<Data = any, Error = any>(...args): SWRResponse<Data, Error> {
  ...
  if (fn === undefined) {
    fn = config.fetcher || etherJsFetcher(config.provider, getSigner(config), config.ABIs);
  }
  ...
  // FIXME merge in only one useEffect
  useEffect(() => {
    if (!config.provider || !getSigner(config) || !config.subscribe || !isAddress(target)) {
      return () => ({});
    }
